...
charges_model=tf.keras.models.load_model("/home/bunnys-weapon/ML web apps/multiple disease prediction web app--------VERSION-1/charges_model.h5")


charges_scaler=pickle.load(open("/home/bunnys-weapon/ML web apps/multiple disease prediction web app--------VERSION-1/charges_scaler.sav","rb"))
...
if selected=="Cardio Diesase Prediction":
	def pred_cardiovascular(inp):
		...
		if pred[0]>0.5:
			return "The patient has cardiac disease"
		else:
...
if selected=="Medical expenses regression":
	def pred_medicalexpenses(inp):
		if inp[1]=="male":
			inp[1]=1
		elif inp[1]=="female":
			inp[1]=0
		...
		if inp[5]=="southeast":
			inp[5]=1
		elif inp[5]=="southwest":
			inp[5]=2
		elif inp[5]=="northeast":
			inp[5]=4
		elif inp[5]=="northwest":
			inp[5]=3
			
		
		inp=np.asarray(inp)
		inp=inp.reshape(1,-1)
		inp=charges_scaler.transform(inp)
		pred=charges_model.predict(inp)
		return pred
	...
	result=""
	if st.button("medical expenses prediction"):
		inp=[	int(age),sex,float(bmi),int(children)	,smoker,region]
		result=pred_medicalexpenses(inp)
	st.success(result)
